...
struct LayoutPageView: View {
  ...
  var body: some View {
    ...
    ZoomableScrollView(
      ...
      Text(
        "\(DateHelper.dayOfWeek(for: selectedDate)), \(DateHelper.formatFullDate(selectedDate)) (\(selectedCategory.rawValue)) - \(DateHelper.formatTime(currentTime))"
      )
    ...
    .onChange(of: selectedCategory) { old, newCategory in
      ...
      reloadLayout(newCategory, activeReservations)
        ...
        clusterManager.recalculateClustersIfNeeded(
            for: activeReservations, tables: layoutUI.tables, combinedDate: combinedDate, oldCategory: old,
    ...
    .onChange(of: currentTime) { old, newTime in
      ...
      clusterManager.recalculateClustersIfNeeded(
        for: activeReservations, tables: layoutUI.tables, combinedDate: combinedDate, oldCategory: selectedCategory,
        selectedCategory: selectedCategory, cellSize: gridData.cellSize)
  ...
  private func loadCurrentLayout() {
    ...
    if !layoutUI.isConfigured {
      layoutUI.configure(
        ...
        layoutUI.tables = layoutServices.loadTables(for: combinedDate, category: determinedCategory)
    }

    if !clusterManager.isConfigured {
      clusterManager.configure(
        store: store, reservationService: reservationService, clusterServices: clusterServices,
        layoutServices: layoutServices)
    ...
    clusterManager.recalculateClustersIfNeeded(
        for: activeReservations, tables: layoutUI.tables, combinedDate: combinedDate, oldCategory: determinedCategory,
